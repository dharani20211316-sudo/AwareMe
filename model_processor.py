...
def _load_patterns_from_mongo():
    """Load expanded regex patterns from MongoDB historyDB.keywords collection."""
    try:
        mc = MongoClient(MONGO_URI, serverSelectionTimeoutMS=3000)
        db = mc["historyDB"]
        doc = db["keywords"].find_one({"_id": "expanded_patterns_v2"})
...
def get_expanded_patterns():
    """Load patterns from MongoDB historyDB.keywords. Raises if unavailable."""
    ...
    if mongo_patterns:
        ...
        return _cached_patterns
    raise RuntimeError("Could not load detection patterns from MongoDB historyDB.keywords. Run seed_keywords.py first.")
